...
dotenv.config()
route.get('/', (req, res) => {
    res.redirect('https://scheduleros.herokuapp.com/login')
})
...
route.get('/concept', (req, res) => {
    res.sendFile(path.join(__dirname, '../views/concept.html'))
})
route.get('/concept/project.pdf',(req,res)=>{
    res.sendFile(path.join(__dirname, '../files/project.pdf'))
})
...
function sendmail(tomailid, hash, fp) {
    ...
    if (fp == 1) {
        mailDetails = {
            ...
            text: 'Reset your password by clicking on the link (link is valid upto five minutes only) ' + 'https://scheduleros.herokuapp.com/forgot?id=' + hash + '&tm=' + tm + '&mail=' + tomailid,
        };
    } else if (fp == 0) {
        mailDetails = {
            from: process.env.mail,
            ...
            text: 'Verify your account by clicking on the link ' + 'https://scheduleros.herokuapp.com/activate?id=' + hash + '&mail=' + tomailid + '&tm=' + tm + ' .' + 'This Link will expire in 10 minutes',
        };
    }
